...
### UI / Frontend Changes
- **DateRangeSlider — Full Dual-Thumb Overhaul**: Completely rewrote `DateRangeSlider.tsx` and fixed `index.css` slider CSS:
  - **Left thumb unlocked**: Fixed the classic dual-range slider bug where the end thumb (last in DOM) sat on top of the start thumb everywhere. Fixed by adding `pointer-events: none` on the `<input>` track (via `.slider-thumb` class) and `pointer-events: all` only on the `::slider-thumb` pseudo-element, so both thumbs are independently draggable. Added dynamic z-index swap (`startThumbZ`/`endThumbZ`) so when thumbs meet at the same position the start thumb rises to the top and can be dragged left.
  - **CUSTOM badge**: A highlighted amber "CUSTOM" badge appears in the Quick Range row whenever the active date window doesn't match any preset (1D/3D/7D/14D/FULL), giving the user clear feedback that a custom range is active.
  - **Start ≤ End enforcement**: Both slider `onChange` and the new native date picker handlers clamp and guard against invalid ranges in both directions — selecting a start after the current end pushes the end up, and vice versa.
  - **Date picker calendar**: Clicking the **From** or **To** date labels opens the browser's native calendar date picker (via hidden `<input type="date">` + `showPicker()` API). The date picker `min`/`max` attributes are dynamically bound to the available coverage window and the other thumb's current date, preventing out-of-bounds or crossed selections.
