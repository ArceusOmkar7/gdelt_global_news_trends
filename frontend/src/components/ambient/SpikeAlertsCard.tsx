...
export const SpikeAlertsCard: React.FC = () => {
  ...
  return (
    <div className="glass-panel w-full overflow-hidden border-cyber-red/20">
      {/* Header */}
      <button
      ...
      {!isCollapsed && (
        <div className="p-3 space-y-2">
          ...
          {spikes.map((s: SpikeAlertEntry) => (
            <div 
              key={s.country_code}
              className="p-2 rounded bg-cyber-red/5 border border-cyber-red/10 flex flex-col gap-1"
            >
              ...
              {anomalies[s.country_code]?.is_anomaly && (
                <div className="mt-1 flex items-center gap-1.5">
                  <span className="text-[8px] font-mono bg-amber-500/20 text-amber-400 px-1 rounded border border-amber-500/30 uppercase">
                    ◈ Anomaly
                  </span>
          ...
          {anomalyOnlyCCs.map((cc) => (
            <div 
              key={cc}
              className="p-2 rounded bg-amber-500/5 border border-amber-500/10 flex flex-col gap-1"
            >
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-mono font-bold text-white/90">
                  ◈ {cc}
