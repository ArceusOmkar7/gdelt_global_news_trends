...
import { Activity, RefreshCcw, SlidersHorizontal, Timer, HeartPulse } from 'lucide-react';
...
export const SystemControlPanel = () => {
  ...
  const onAutoRefreshChange = (e: ChangeEvent<HTMLInputElement>) => {
    setAutoRefreshEnabled(e.target.checked);
  };

  const onFetchIntervalChange = (e: ChangeEvent<HTMLSelectElement>) => {
    setFetchIntervalSeconds(Number(e.target.value));
  };

  const onHealthIntervalChange = (e: ChangeEvent<HTMLSelectElement>) => {
    setHealthPollIntervalSeconds(Number(e.target.value));
  };
  ...
  return (
    <div className="glass-panel p-4 w-80 space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <SlidersHorizontal size={14} className="text-cyber-blue" />
      ...
      <div className="space-y-2 text-[11px] font-mono text-white/75">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <select
            ...
            onChange={onFetchIntervalChange}
            className="bg-surface-900 border border-white/20 rounded px-2 py-1 text-[11px] font-mono"
          >
            {FETCH_INTERVAL_OPTIONS.map((seconds) => (
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <select
            value={healthPollIntervalSeconds}
            onChange={onHealthIntervalChange}
            className="bg-surface-900 border border-white/20 rounded px-2 py-1 text-[11px] font-mono"
          >
            {HEALTH_INTERVAL_OPTIONS.map((seconds) => (
              <option key={seconds} value={seconds}>
                {seconds}s
              </option>
            ))}
      ...
      <div className="space-y-2 border-t border-white/10 pt-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="data-ink">System Health</span>
          </div>
        ...
        {healthQuery.isError ? (
          <div className="text-[10px] font-mono text-cyber-red/90">
          ...
          <div className="grid grid-cols-2 gap-2 text-[10px] font-mono text-white/70">
            ...
            <div>
              Uptime: {health ? `${Math.round(health.uptime_seconds)}s` : '--'}
            </div>
            <div>
              BQ: {health?.bigquery.connected ? 'OK' : 'DOWN'}
            </div>
            <div>
            ...
            <div className="col-span-2 text-white/55">
              Hot/cold cutoff: {health?.hot_tier.cutoff_days ?? '--'} days
            </div>
            <div className="col-span-2 text-white/55">
              Filters: {dateRange[0]} to {dateRange[1]}{eventRootCode ? ` | code ${eventRootCode}` : ''}
            </div>
          </div>
        )}
      </div>
      ...
      <div className="space-y-2 border-t border-white/10 pt-3">
        ...
        {settingsQuery.isError ? (
          <div className="text-[10px] font-mono text-cyber-red/90">
            Runtime settings endpoint unavailable.
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-2 text-[10px] font-mono text-white/70">
            ...
            <div className="col-span-2 text-white/55">
              Realtime fetch cadence: every {settingsQuery.data?.realtime_fetch_interval_minutes ?? '--'} min
            </div>
            <div className="col-span-2 text-white/55">
              Cron UTC: daily {settingsQuery.data?.daily_batch_cron_utc ?? '--'} | nightly {settingsQuery.data?.nightly_ai_cron_utc ?? '--'}
            </div>
          </div>
        )}
      </div>
